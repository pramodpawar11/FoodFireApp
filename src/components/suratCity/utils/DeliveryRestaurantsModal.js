...
const DeliveryRestaurantsModal = ({ closeModal,deliveryRestaurants,topRestaurants }) => {
    const selectedValue = useSelector((store)=>store.filterData.values);
    const dispatch = useDispatch();
    console.log(topRestaurants);
    if(selectedValue==="rating"){
        const sortedResult = [...deliveryRestaurants].sort((a, b) => parseFloat(b.info.avgRating) - parseFloat(a.info.avgRating));
        dispatch(addrating(sortedResult));
    }
    return (
